...
export class MeasurementDrawing extends DrawingPrimitive {
    private type: 'price' | 'time' | 'both';

    constructor(type: 'price' | 'time' | 'both' = 'both', options?: DrawingOptions) {
        super(options);
        this.type = type;
    }
    ...
    draw(ctx: CanvasRenderingContext2D, chart: IChartApi, series: ISeriesApi<any>): void {
        ...
        // Duration (rough estimate based on bars if possible, or just time)
        const timeDiffMs = Math.abs(Number(p2.time) - Number(p1.time)) * 1000;
        ...
        let label = '';
        if (this.type === 'price' || this.type === 'both') {
            label += `${priceDiff.toFixed(2)} (${pricePercent.toFixed(2)}%)`;
        }
        if (this.type === 'time' || this.type === 'both') {
            if (label) label += ' | ';
